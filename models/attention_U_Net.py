...
class UNet(nn.Module):
    def __init__(self,use_time_scaling=True):
        ...
        self.multiplier_time_scaling=torch.zeros(1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        if (use_time_scaling):
            self.multiplier_time_scaling=torch.ones(1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    ...
    def forward(self, input_image, t, s):
        ...
        out = self.conv_output(self.bn(y1))*((-1-t.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1))*self.multiplier_time_scaling+skip_weights_network_output[:,8:9, None, None]*(0.01+1-self.multiplier_time_scaling))+input_image*(skip_weights_noise[:,8:9, None, None]*(0.01+1-self.multiplier_time_scaling)+1)
        return out
